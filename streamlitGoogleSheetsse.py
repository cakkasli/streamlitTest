...
if data is not None and not data.empty:
    ...
    upper_y_limit = max(ax.get_ylim()[1] for ax_row in axes for ax in ax_row)
    ...
    for session, color in zip(session_numbers, colors):
        ...
        if session % 3 == 1:
            axes[0, 0].text(
                ...
                upper_y_limit + 1500,  # Slightly above the y-axis limit for alignment
                str(session), 
            ...
            axes[0, 0].text(
                session_data["ID"].iloc[-1],  # Last ID in the session
                upper_y_limit,  # Slightly above the y-axis limit for alignment
                "ı", 
    ...
    for session, color in zip(session_numbers, colors):
        ...
        if session % 3 == 1:
            axes[0, 1].text(
                session_data["ID"].iloc[-1],  # Last ID in the session
                803.0,  # Slightly above the y-axis limit for alignment
                str(session), 
                fontsize=10, fontweight='bold', color='black',
                ha='center', va='bottom'  # Bottom-align the text
            )
            axes[0, 1].text(
                session_data["ID"].iloc[-1],  # Last ID in the session
                800.5,  # Slightly above the y-axis limit for alignment
                "ı", 
                fontsize=10, fontweight='normal', color='black',
                ha='center', va='bottom'  # Bottom-align the text
            )
